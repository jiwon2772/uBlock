...
if (typeof vAPI !== 'object') {
  // vapi-client.js를 실행하면서 vAPI객체가 만들어 졌기 때문에 Object가 존재한다.
...
vAPI.domFilterer = (function() {
  ...
  (function() {
    // vAPI에서 제공하는 hideNode가 있는지 확인.
    // 제공해주면 return, 제공하지 않으면 정의해준다.
    ...
    var batchProcess = function() {
      timer.clear(); // 시간 초기화
      var uid_ = uid; // uid 저장
      for (var node of changedNodes) { // changedNodes 안에있는 각 node마다 반복작업
        if ( // 만약 현재 확인하는 node객체의 uid값이 undefined거나 node의 width, height
          // 증 하나라도 0인 경우 다음 node로 건너뛴다.
          node[uid_] === undefined ||
          node.clientHeight === 0 || node.clientWidth === 0
        ) {
          continue;
        }
        // 현재 node의 uid값이 존재하는 경우
        var attr = node.getAttribute('style'); //'style' attribute의 값을 가져온다.
        if (attr === null) { // attr이 빈 값을 경우 empty string으로 설정해줌.
          attr = '';
        } else if (
          // 마지막에 ;이 존재하지 않을 경우 추가해준다.
          attr.length !== 0 &&
        ...
        // 'style' attribute의 기존 값에 + 'display: none !important;'를 더 해주어
        // 화면에서 보이지 않게한다.
      }
      changedNodes.clear(); // 모든 node에 대한 진행이 끝났으므로 changedNodes를 초기화
    };
    ...
    platformHideNode = function(node) {
      if (uid === undefined) { //uid가 undefined면 radom하게 받아온다.
        uid = vAPI.randomToken();
        timer = new vAPI.SafeAnimationFrame(batchProcess);
        //timer를 새로 등록한다.
      }
      if (node[uid] === undefined) { //node의 uid값이 undefined면 node[uid]값에
        // 'style' attribute의 값을 넣어준다.
...
// dom이 로딩된 직후 실행되는 부분 (여기부터가 Ad Blocking의 시작인듯하다.)
(function domIsLoading() {
  ...
  var responseHandler = function(response) {
    ...
    // cosmetic filtering을 사용하지 않는 경우
    if (response.noCosmeticFiltering) {
      vAPI.domFilterer = null;
      vAPI.domSurveyor = null;
    } else { // cosmetic filtering을 사용하는 경우
    ...
    // dom에서 가장 최상단 node를 받아와 parent에 저장
    var parent = document.head || document.documentElement;
    if (parent) { // parent가 존재하는 경우
      var elem, text;
...
vAPI.domCollapser = (function() {
  ...
  var send = function() {
    ...
    messaging.send(
      // hostname에 해당하는 filter를 요청하는 message를 전달한다.
      'contentscript', {
        what: 'filterRequests',
  ...
  var add = function(target) {
    var tag = target.localName; // 현재 확인중인 대상의 localName이 무엇인지 확인
    var prop = src1stProps[tag]; // localName에 따른 src이름을 return
    if (prop === undefined) {
      return;
    }
    // https://github.com/chrisaljoudi/uBlock/issues/174
    // Do not remove fragment from src URL
    var src = target[prop]; // 현재 확인 중인 대상의 src url을 가져옴
    ...
    // http가 안붙어있는 경우는 return시킨다.
    // (내부 resources는 광고가 아니라고 판단하는 듯하다.)
  ...
  var start = function() {
    ...
    // dom에 존재하는 모든 image테그를 가져온다. (domCollection으로 반환됨)
    var elems = document.images || document.getElementsByTagName('img'),
      i = elems.length,
      elem;
    while (i--) {
      elem = elems[i];
      //elem.complete를 통해 현재 보려하는 image의 다운이 완료했는지 확인
      if (elem.complete) {
        add(elem); //확인된 경우 add함수를 통해서 pendingRequests와 roundtripRequests에 삽입
      }
    }
    // 'embed' tag에 대한 정보를 가져와서 더해준다.
    addMany(document.embeds || document.getElementsByTagName('embed'));
    // 'object' tag에 대한 정보를 가져와서 더해준다.
    addMany(document.getElementsByTagName('object'));
    // 'iframe' tag에 대한 정보를 가져와서 더해준다.
