...
µBlock.URI = (function() {
  ...
  URI.set = function(uri) {
    if (uri === undefined) { // 인자가 undefined일 경우 초기화 한 URI객체를 return
      return reset(URI);
    }
    var matches = reRFC3986.exec(uri);  // 인자와 reRFC3986(URI형식에 해당하는 정규표현식)을 비교
    if (!matches) {
      return reset(URI); // 일치하지 않으면 초기화 한 URI객체를 return
    }

    // match할 경우 각각의 구성요소 마다 label을 붙여준다.
    ...
    //만약 authority와 path에 해당값이 존재하지 않으면 URI.path의 값을 '/'로 초기화
    ...
    // case for µBlock)
    // authority를 hostname과 port로 쪼갤 수 있는 경우는 쪼개어 각각의 변수에 저장
