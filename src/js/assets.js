...
µBlock.assets = (function() {
  ...
  api.addObserver = function(observer) {
    if (observers.indexOf(observer) === -1) { //기존 observer리스트에 존재하는지 확인 후 없으면 추가
  ...
  // observer리스트에 존재하는 모든 observer의 return값이 false인지 true인지 확인
  ...
  api.fetchText = function(url, onLoad, onError) {
    var isExternal = reIsExternalPath.test(url),
      ...
      // external주소의 체계와 맞으면 url을 그대로 사용하고, 다를경우 크롬API로 부터 현재 url을 받아와 적용
    ...
    // url이 external주소의 체계와 맞는지 확인
    if (isExternal) {
      var queryValue = '_=' + Math.floor(Date.now() / 7200000);
      //현재 url에 '?'이 없으면 새롭게 추가하고 없으면 &를 추가 후 queryValue를 더함
    ...
    var contentLoaded = 0,
      ...
      // assetFetchTimeout이 존재하지 않을경우 30000이 default값이 된다.
      timeoutTimer,
    ...
    var onLoadEvent = function() {
      ...
      details.content = this.responseText; //응답받은 text를 details.content에 채운다.
      onLoad(details);
    };
  ...
  var migrate = function(callback) {
    ...
    var onEntries = function(bin) {
      ...
      for (var oldKey in entries) {
        if (oldKey.endsWith('assets/user/filters.txt')) {
          continue;
        }
  ...
  **/
  ...
  var updateNext = function() {
    ...
    var updatedOne = function(details) {
      if (details.content !== '') {
        ...
        fireNotification('asset-update-failed', {
  ...
  var updateDone = function() {
    ...
    updaterAssetDelay = updaterAssetDelayDefault;
    fireNotification('after-assets-updated', {
      assetKeys: assetKeys
    });
  };
